...
namespace WindowsAppPackage
{
    public class PackageGrabber : IGrabber
    {
        ...
        public Task<List<KeyValuePair<string, string>>> Grab(AnalysisContext analysisContext)
        {
            ...
            Result<string> manifestResult = SevenZipService.ExtractEntryToString(analysisContext.FilePath, "AppxManifest.xml");
            ...
            {
                _logger.Log(LogLevel.Error, $"{Path.GetFileName(analysisContext.FilePath)}\tFailed to parse AppxManifest.xml: {ex.Message}");
                return Task.FromResult(properties);
            }
        }
